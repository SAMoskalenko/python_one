...
def withdraw_money(person, money):
    if money < 0:
        ...
        if person['money'] - money >= 0:  # не равно а больше или равно нуля
            person['money'] -= money
...
def start():
    data = input('Введите номер карты и пин код через пробел:')
    try:
        card_number, pin_code = data.split()
        ...
        if person and is_pin_valid(person, pin_code):
            while True:
                choice: int = int(input('Выберите пункт:\n'
                                        '1. Проверить баланс\n'
                                        '2. Снять деньги\n'
                                        '3. Выход\n'
                                        '---------------------\n'
